...
```{r, eval=FALSE}
library(plotly)
plot_ly(mtcars, x = ~wt, y = ~mpg, type = "scatter")  ## mode = "markers" in lecture
```
...
### ggplotly() turns any plot into a plot_ly html widget. Nice.

### plotly_POST() posts it at the plot.ly website
     ## have to set up some API keys first
```
...
Boxplots are wonderful for comparing how different datastes are distributed.
